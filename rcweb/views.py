...
from .models import SignUp,UserProfile,Newsletter,Contact,Instructor,Workshop,Courses,Ngo
def home(request):
...
def admintrack(request):
    if request.method == 'POST':
        id=int(request.POST.get("id"))
        if(id == 1):
            m=Newsletter.objects.all().reverse()
        elif(id == 2):
            m=Contact.objects.all().reverse()
        else:
...
def adminenroll(request):
    return render(request,"adminenroll.html")
...
def addworkshop(request):
    ...
    if request.method == 'POST':
        ...
        df=Workshop(etitle=etitle,edesc=edesc,edoclink=edoclink,einame=einame,eidesc=eidesc,eilink=eilink,edate=edate,etime=etime)
        df.save()
...
def upworkshop(request):
    ...
    if request.method == 'POST':
        ...
        edate=request.POST.get("edate")
        etime=request.POST.get("etime")
        Workshop.objects.filter(etitle=etitle).update(etitle=etitle,edesc=edesc,edoclink=edoclink,einame=einame,eidesc=eidesc,eilink=eilink,edate=edate,etime=etime)
...
def addcourses(request):
    ...
    if request.method == 'POST':
        ...
        df=Courses(ctitle=ctitle,cdesc=cdesc,cdoclink=cdoclink,ciname=ciname,cidesc=cidesc,cilink=cilink,csdate=csdate,cedate=cedate,ctime=ctime)
        df.save()
...
def upcourses(request):
    ...
    if request.method == 'POST':
        ...
        cedate=request.POST.get("cedate")
        ctime=request.POST.get("ctime")
        Courses.objects.filter(ctitle=ctitle).update(ctitle=ctitle,cdesc=cdesc,cdoclink=cdoclink,ciname=ciname,cidesc=cidesc,cilink=cilink,csdate=csdate,cedate=cedate,ctime=ctime)
...
def addngo(request):
    ...
    if request.method == 'POST':
        ...
        df=Ngo(ngoname=ngoname,ngodetails=ngodetails,ntitle=ntitle,ndesc=ndesc,ndoclink=ndoclink,niname=niname,nidesc=nidesc,nilink=nilink,nsdate=nsdate,nedate=nedate,ntime=ntime)
        df.save()
...
def upngo(request):
    ...
    if request.method == 'POST':
        ...
        nedate=request.POST.get("nedate")
        ntime=request.POST.get("ntime")
        Ngo.objects.filter(ntitle=ntitle).update(ngoname=ngoname,ngodetails=ngodetails,ntitle=ntitle,ndesc=ndesc,ndoclink=ndoclink,niname=niname,nidesc=nidesc,nilink=nilink,nsdate=nsdate,nedate=nedate,ntime=ntime)
